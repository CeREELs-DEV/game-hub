...
const fairyTaleRoot = new URL('../games/fairy-tale/', import.meta.url);

function fairyTaleUrl(path = '') {
  return new URL(path, fairyTaleRoot);
}
...
test('renders the six playable game cards and removes coming soon content', () => {
  ...
  assert.deepEqual(getCardAnchors(), [
    { label: 'STORY GOGGLES', href: './games/story-goggles/' },
    { label: 'MATTER OF PERSPECTIVE', href: './games/perspective/' },
    { label: 'Life is a Fairy Tale', href: './games/fairy-tale/' },
    { label: 'Connections', href: './games/connections/' },
    { label: 'Coupang', href: './games/coupang/' },
    { label: 'Escape Room', href: './games/escape-room/' },
  ]);
...
test('publishes the functional local demo as a repo-relative static app', () => {
  const demoIndexUrl = fairyTaleUrl('index.html');
  ...
  const demoBundle = readFileSync(fairyTaleUrl(scriptMatch[1]), 'utf8');
...
test('publishes the v3 fairy tale prototype diary samples and journal image', () => {
  ...
  const sampleImages = readdirSync(fairyTaleUrl('assets/'))
    .filter((name) => /^sample-moment-.*\.jpg$/.test(name))
    .sort();
  assert.equal(sampleImages.length, 3);
  assert.deepEqual(
    sampleImages.map((name) => assetSha256(`../games/fairy-tale/assets/${name}`)).sort(),
    [
...
test('publishes the prototype photo step instead of the camera overlay entry state', () => {
  ...
  const demoBundle = readFileSync(fairyTaleUrl(scriptMatch[1]), 'utf8');
  const demoCss = readFileSync(fairyTaleUrl(cssMatch[1]), 'utf8');
